...
interface AnalyticsResponse {
  ...
  message: string;
  data: {
    rawLogs: Array<{ id: string; ip: string; userAgent: string; createdAt: string }>;
    chartData: Array<{ date: string; _count: { id: number } }>;
  };
}

export async function shortenUrl(url: string): Promise<ShortenResponse['data']> {
...
export async function getAnalytics(shortId: string): Promise<AnalyticsResponse['data']> {
